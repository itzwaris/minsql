...
impl StorageEngine {
    ...
    pub fn create_table(&self, table_name: &str, schema: &str) -> Result<()> {
        ...
        if result != 0 {
            anyhow::bail!("Failed to create table '{}': error code {}", table_name, result);
        }
        self.checkpoint()?;
        
        tracing::info!("Successfully created table '{}' in system catalog", table_name);
        Ok(())
    }

    pub fn insert_row(&self, table_name: &str, data: &[u8]) -> Result<u64> {
        tracing::debug!("Inserting row into table '{}', data size: {} bytes", table_name, data.len());
        ...
        if result != 0 {
            anyhow::bail!("Failed to insert row into '{}': error code {}", table_name, result);
        }
        self.wal_flush()?;
        
        tracing::debug!("Successfully inserted row with ID {} into '{}'", row_id, table_name);
        Ok(row_id)
    }

    pub fn update_rows(&self, table_name: &str, predicate: &str, data: &[u8]) -> Result<usize> {
        tracing::debug!("Updating rows in table '{}' matching: {}", table_name, predicate);
        ...
        if result != 0 {
            anyhow::bail!("Failed to update rows in '{}': error code {}", table_name, result);
        }
        self.wal_flush()?;
    ...
    pub fn delete_rows(&self, table_name: &str, predicate: &str) -> Result<usize> {
        tracing::debug!("Deleting rows from table '{}' matching: {}", table_name, predicate);
        ...
        if result != 0 {
            anyhow::bail!("Failed to delete rows from '{}': error code {}", table_name, result);
        }
